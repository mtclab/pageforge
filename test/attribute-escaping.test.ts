...
const RENDER_PATHS: { label: string; opts: RenderOptions }[] = [
  ...
  { label: 'business page', opts: { bizHero: true, heroCta: true } },
];
...
describe('attribute escaping (parsed DOM)', () => {
  ...
  it('the published business page is held to the same rule', () => {
    // bizHtml wraps the engine output with its own inline-styled chrome, so
    // only the handler rule applies here - the chrome is ours, not the user's.
    const doc = parse(bizHtml(hostile, false));
    expect(eventHandlers(doc)).toEqual([]);
    const draft = parse(bizHtml(hostile, true, true, { action: '/c', token: 't' }, false, {
      siteId: 's', token: 't',
    }));
    expect(eventHandlers(draft)).toEqual([]);
  });
  ...
  it('quotes in every attribute-bound field come back out as text, not markup', () => {
    ...
    const doc = parse(renderSite(hostile, THEMES[0]!, { heroCta: true }).html);
    ...
    expect(gallery[0]!.getAttribute('src')).toBe('assets/g1.jpg" onerror="alert(\'g1\')');
